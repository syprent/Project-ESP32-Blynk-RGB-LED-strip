...
En lo personal elegí [Blynk](https://blynk.io/) para operar a través de internet dada la familiariedad y simplicidad de su uso, además que ofrece opciones multiplataforma. Sé que existen alternativas mediante ESPHome o páginas web, pero en este caso cumple con su propósito.
...
<img src="img\front_circuit.png" alt="Circuito original front" width="500"/>
<img src="img\back_circuit.png" alt="Circuito original back" width="350"/>

## Control mediante señales IR hacia el chip de la tira de luces (opción descartada)

Hay muchas formas de pelar un conejo, de la misma forma en la que uno puede elegir cómo y dónde conectar el chip para manejar las luces. En un principio quise que la ESP32 emulara el envío de señales infrarrojas del sensor hacia el chip, pero surgieron un par de problemas debido al diodo Zenner, que, al menos en mi caso, me generaba mediciones negativas de voltaje que la placa y el regulador no podían manejar, así que fue una opción descartada.

De todas formas, si te interesa explorar la alternativa de señales infrarrojas, el protocolo de comunicación IR de las luces led resultó ser NEC en mi caso. Hay varios foros con tutoriales y códigos que te permiten decodificar las señales enviadas desde el control con **IRreciv**. Mediante ensayo y error, estos fueron los comandos del control decodificados en formato HEX. Recomiendo verificar y probar por tu cuenta si te interesa decodificar e implementar de esta manera las señales.
...
Para las conexiones de los cables RGB entre la placa ESP32 y la tria de luces LED, es necesario añadir componentes a manera de "válvula" o "compuerta" para su protección. Hay dos formas de hacerlo, la primera es mediante MOSFETs tipo N, en los que destaca el uso de los [TIP120 conectados con resistencias](https://learn.adafruit.com/rgb-led-strips/usage).

La segunda forma (y utilizada en el proyecto), simplifica bastante el circuito con el driver ULN2003, el cual saqué de un motor paso a paso que no estaba utilizando. Su implementación es similar a [este proyecto donde utiliza el sensor IR](https://www.engineersgarage.com/how-to-build-an-ir-remote-operated-rgb-led-strip-using-arduino/).
...
El código fue de elaboración propia asistido por Copilot, buscando implementar las mismas funciones del control IR a través de la nube.
...
Luego en la app de Blynk, aparece automáticamente el template creado. Allí se añaden los mismos elementos que el dashboard web, añadiendo el color picker de Zegbra, con cada datastream asociado a los slider R, G y B en los pines V0, V1 y V2 respectivamente.
